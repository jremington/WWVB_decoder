...
// ****************
// 12/29 version 15: added minimum value for positive bit ID: 40% correlation (NEEDS TWEAKING, up this to reduce single bit errors)
// ****************
...
// CRITICAL parameter for bit error detection, now set to 40% of max positive correlation value.
#define MIN_VALID_BIT_CORR (40*SAMPLE_HZ/100)      // define valid bit = minimum % correlation with bit template
...
ISR(TIMER1_COMPA_vect) {  // called every 20mS @50 Hz
  ...
  if (sampleCounter >= SAMPLE_HZ)                // full second of sampling?
  {
    ...
    byte retval = ERRBIT;
    if (max >= MIN_VALID_BIT_CORR) retval = type;   // CRITICAL THRESHOLD!
...
void setup() {
  ...
  while (!Serial);
  Serial.println();
  Serial.println("WWVB_15 added minimum valid bit correlation, 50 Hz, double sync detection, template cross correlation, prescaler 8");
